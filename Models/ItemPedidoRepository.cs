...
namespace Farmacia.Models
{
    public class ItemPedidoRepository : Repository
    {
        ...
        {
            conexao.Open();
            string sql = "INSERT INTO ItemPedido (id_pedido, id_medicamento, quantidade)"
            +" VALUES (@idPedido, @idMedicamento, @quantidade)";
         ...
         public List<ItemPedido> Lista(int idPedido)
    {
        ...
        string sql = "SELECT ip.id as IdItemPedido, ip.id_pedido, ip.quantidade," +
        " m.id as IdMedicamento, m.nome, m.valor" + 
        " FROM itempedido ip JOIN medicamento m ON ip.id_medicamento = m.id "+
